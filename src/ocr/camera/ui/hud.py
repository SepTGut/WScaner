...
class HUDRenderer:
    ...
    def render(
        ...
        cv2.putText(frame, f"ENGINE: {current_engine.upper()}", (195, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.46, (0, 220, 255), 1)

        doc_detected = current_edge_density >= min_edge_density
        doc_label = "DOC: SIAP [OK]" if doc_detected else "DOC: MENCARI..."
        doc_color = (80, 240, 100) if doc_detected else (140, 140, 150)
        cv2.putText(frame, doc_label, (375, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.44, doc_color, 1)

        res_badge = f"RES: {W}x{H}"
        cv2.putText(frame, res_badge, (545, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.44, (200, 215, 230), 1)

        fps_color = (80, 240, 100) if fps >= 22.0 else (0, 220, 255) if fps >= 14.0 else (60, 60, 240)
        cv2.putText(frame, f"FPS: {fps:.1f}", (695, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.44, fps_color, 1)
        ...
        if W >= 1280:
            cv2.putText(frame, full_status, (18, H - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.52, theme_color, 2)
            cv2.putText(frame, controls_hint, (W - 570, H - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.42, (160, 160, 170), 1)
        else:
            cv2.putText(frame, full_status, (18, H - 27), cv2.FONT_HERSHEY_SIMPLEX, 0.48, theme_color, 2)
